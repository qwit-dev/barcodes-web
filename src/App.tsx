...
function App() {
    ...
    return (
        <div>
            <div id="full-screen-barcode-modal" className="z-50 w-screen h-screen p-[10px] absolute bg-black" style={{display: fsBarcode == null ? "none" : "block"}} onClick={() => {setFsBarcode(null)}}>
                <Barcode value={fsBarcode?.value} format={fsBarcode?.format} className="w-full h-full" />
            </div>
                    ...
                    barcodes ? barcodes.map((barcode: BarcodeType) => {
                        return (
                            <div key={barcode.id} onClick={() => {setFsBarcode(barcode)}}
                                className="bg-neutral-800 flex flex-col items-center justify-center rounded-3xl p-[20px] cursor-pointer" id={barcode.id}>
                                {/*<img src={qr1} alt={barcode.value} className="rounded-xl"/>*/}
                                <Barcode value={barcode.value} format={barcode.format} className={"rounded-xl"}  />
